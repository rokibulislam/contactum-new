...
class FormHistory {
    ...
    public function record( $form_id, $old_fields, $new_fields ) {
        // A form's very first save has nothing before it to speak of — the
        // "old" state is an empty form. Recording that as a history entry
        // would just be a trap: it's the only entry a freshly-built form
        // has, "Restore" on it wipes every field the user just added, and
        // nothing distinguishes it from a normal, safely-restorable entry.
        if ( empty( $old_fields ) ) {
            return;
        }
